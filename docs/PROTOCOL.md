...
4. The first upload contains exactly one AUTH frame and no other frames. Stream and frame sequence
   ...
   URL-decoded username, colon and password. The module's own authentication and destination policy apply. AUTH must fit its first 4096-byte cell.
...
order: the domain string naivefox-site, the 32 raw SHA-256 digest bytes of
...
The NaiveFox access configuration supplies credentials, ordered ACL rules,
ports, an optional upstream and dial timeout. Explicit ACL rules precede private
network exclusions and a final allow rule. A configured ordinary HTTP CONNECT
or SOCKS5 upstream owns destination DNS and policy. The default dial timeout is
...
Only current `naivefox` is supported. Public resources carry no
...
HTTP/1.1 TLS, the session cookie and matching HTTPS Origin.

Strict H3 instead opens GET /api/stream over HTTP/3. Its persistent response
has application/octet-stream MIME type and no fixed content length. Every
downstream cell has a four-byte unsigned big-endian length prefix. The decoder
accepts split prefixes/cells and multiple cells per read, validating capacity
before allocation.

Upstream H3 cells use POST /api/upload, one cell per request, with at most eight
active request bodies and eight retained out-of-order sequences, at most 1 MiB
of retained cell bodies per carrier. CREDIT-only uploads occupy at most three
request slots; payload and lifecycle frames can use the remaining slots. Responses are
204 only after in-order cell application. A duplicate, out-of-window sequence,
gap timeout or cancellation ends the carrier. The client never replays a
request at the application layer. The carrier uses Neqo HTTP/3 and never WSS/TCP.

Both adapters share directional cell sequences and mux state. Only one
persistent downstream may attach; startup requests cannot resume afterward.

## Sustained carrier bounds

An H2 binary WebSocket message or H3 POST contains one complete NFOX cell. Reserved header
...
coalescing interval remains fixed while new data arrives. There is one bounded writer; a
response is encoded exactly once. No data is replayed after write failure.

An idle H3 GET starts with one immediate empty 512-byte cell so native body
callbacks can make the stream ready. Later idle heartbeats use 512-byte cells
after 25 seconds. Input has a 75-second
...
consume its single NFOX application-message budget.
...
H3 uses the smallest 512-byte cell when all queued data and framing fit. Larger
H3 cells are capped at 64 KiB to limit complete-message delivery latency.
